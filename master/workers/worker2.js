...
module.exports = {
    ...
    execute: async (job, updateStageStatus, sleep) => {
        console.log(`🐍 [Worker 2 - Node.js] Starting job #${job.id} for ${job.repo_name} (Attempt ${job.retry_count + 1})`);
        ...
        try {
            ...
            if (isSuccess) {
                ...
                console.log(`🐍 [Worker 2] Finished job #${job.id} -> ✅ SUCCESS`);
                ...
                if (job.retry_count < MAX_RETRIES) {
                    ...
                    console.log(`🐍 [Worker 2] Job #${job.id} failed. 🔄 Returning to queue for retry...`);
                } else {
                    db.run(`UPDATE jobs SET status = 'failed' WHERE id = ?`, [job.id]);
                    console.log(`🐍 [Worker 2] Finished job #${job.id} -> ❌ FAILED (Max retries reached)`);
                }
            }
        } catch (err) {
            console.error(`🐍 [Worker 1] Execution error:`, err);
        }
    }
};