    ...
    execute: (job) => {
        ...
        setTimeout(() => {
            ...
            if (isSuccess) {
                ...
                console.log(`🐍 [Worker 2 - Node.js] Finished job #${job.id} -> ✅ SUCCESS`);
            } else {
                ...
                if (job.retry_count < MAX_RETRIES) {
                    ...
                    console.log(`🐍 [Worker 2 - Node.js] Job #${job.id} failed. 🔄 Returning to queue for retry...`);
                } else {
                    // Max retries reached, mark as permanently failed
                    db.run(`UPDATE jobs SET status = 'failed' WHERE id = ?`, [job.id]);
                    console.log(`🐍 [Worker 2 - Node.js] Finished job #${job.id} -> ❌ FAILED (Max retries reached)`);
                }
            }
        }, executionTime);
    }
