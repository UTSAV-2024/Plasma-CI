...
module.exports = {
    execute: (job) => {
        ...
        console.log(`🐍 [Worker 3 - Java] Starting job #${job.id} for ${job.repo_name} (Attempt ${job.retry_count + 1})`);
        ...
        setTimeout(() => {
            ...
            if (isSuccess) {
                ...
                console.log(`🐍 [Worker 3 - Java] Finished job #${job.id} -> ✅ SUCCESS`);
                ...
                if (job.retry_count < MAX_RETRIES) {
                    ...
                    console.log(`🐍 [Worker 3 - Java] Job #${job.id} failed. 🔄 Returning to queue for retry...`);
                } else {
                    // Max retries reached, mark as permanently failed
                    db.run(`UPDATE jobs SET status = 'failed' WHERE id = ?`, [job.id]);
                    console.log(`🐍 [Worker 3 - Java] Finished job #${job.id} -> ❌ FAILED (Max retries reached)`);
                }
            }
        }, executionTime);
    }
